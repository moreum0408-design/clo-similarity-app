...
def get_similarity(p1, p2):
    try:
        ...
        cand = suggest(p1)
        raise KeyError(f"Program not found: {p1}\nDid you mean:\n  - " + "\n  - ".join(cand) if cand else "  (no close matches)")
    try:
        i2 = programs.index.get_loc(p2)
    except KeyError:
        cand = suggest(p2)
        raise KeyError(f"Program not found: {p2}\nDid you mean:\n  - " + "\n  - ".join(cand) if cand else "  (no close matches)")
...
def main():
    ...
    print("\nCLO program similarity — type two program names. Case and format must be identical as spreadsheet (exact or close).")
    print("Type q to quit.\n")
    while True:
        p1 = input("First program: ").strip()
        if p1.lower() == "q": break
        p2 = input("Second program: ").strip()
        if p2.lower() == "q": break
        try:
            score = get_similarity(p1, p2)
            print(f"\nSimilarity = {score:.3f}\n")
