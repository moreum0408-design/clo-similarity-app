...
def main():
    ...
    missing_cols = [c for c in [COL_COURSE, COL_CLO] if c not in df.columns]
    if missing_cols:
        raise ValueError(
            f"Missing required columns in {INPUT_XLSX}: {missing_cols}. "
    ...
    for course, g in df.groupby("course"):
        course_to_level[course] = g["level_key"].iloc[0]
        course_to_clos[course] = g["clo"].drop_duplicates().tolist()

    # Courses per level
    level_to_courses = {}
    for course, lvl in course_to_level.items():
    ...
    def sim_one_to_many(base_clo_text: str, other_clo_texts: list[str]) -> np.ndarray:
        ...
        if emb_type == "st":
            ...
            sims = (base_vec @ other_mat.T).reshape(-1)  # normalized
            return sims.astype(np.float32)
        else:
            # TF-IDF sparse cosine (already normalized)
            base_i = clo_to_idx[base_clo_text]
            base_vec = clo_emb[base_i]
            other_idx = [clo_to_idx[t] for t in other_clo_texts]
            other_mat = clo_emb[other_idx]
            sims = (base_vec @ other_mat.T).A.reshape(-1)
            return sims.astype(np.float32)

    # -----------------------------
    # 1) Course similarity (top 20 within same-level)
    # overall = avg over base CLOs of max similarity to any CLO in other course
    # -----------------------------
    course_rows = []

    courses = list(course_to_clos.keys())
    for base_course in courses:
        base_clos = course_to_clos[base_course]
        lvl = course_to_level[base_course]
    ...
    # -----------------------------
    # 3) DRILLDOWN FILE (THIS IS WHAT FIXES YOUR COURSE CLICK PAGE)
    # For each base_course/base_clo and each top-20 course,
    # store TOP K CLOs INSIDE that clicked course.
    # -----------------------------
    K = 50  # change to 20 if you want exactly 20 rows on drilldown
    ...
    print("\nDONE. Commit/push the new CSVs and redeploy.\n")
